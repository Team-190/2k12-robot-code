 ...
 * @author Greg
 */
public class JogTurretViaJoystick extends CommandBase{

    public JogTurretViaJoystick() {
        requires(turret);
    }
    ...
    protected void execute() {
        ...
        if(Math.abs(desiredMotorVal) > 0.1){
            if((Turret.LEFT_BOUND > position) && (position > -150.0)){
                desiredMotorVal = Math.min(desiredMotorVal, 0.0);
            }else if((Turret.RIGHT_BOUND < position) && (position > -150.0)){

 ...
 * to it moves at least a certain amount in the appropriate direction. May be
 * used with mechanisms that turn continuously.
 * 
 * @author pmalmsten
 */
public class TestChangePositionContinuous extends Command {
    ...
    /**
     * Creates a new TestChangePositionContinuous command which applies the given
     * power setting to the given motor and asserts that the given position sensor
     ...
     *
     * This class differs from TestChangePosition in that it does not assert that
     * the associated mechanism remains within bounds. This should be used with
     * care only on mechanisms that can turn indefinitely.
     *
     * If the mechanism turns all the way around, and thus causes the associated
     * absolute position sensor to wrap-around, this command may fail since the
     * direction that the mechanism traveled from its starting position cannot
     * be determined. To resolve this, reduce the duration that the mechanism
     * is under test (see the duration argument below).
     * 
     ...
     *  move as expected.
     */
    public TestChangePositionContinuous(String name,
                                        Subsystem system,
    ...
    public TestChangePositionContinuous(String name,
                              ...
                              double expectedMovement,
                              double duration) {
        this(name, system, output, input, powerToApply, expectedMovement, duration, ProblemManager.getInstance());
    }
